...
{
    public class PLA : Ordonnancement
    {
        ...
        public override async Task<int> Executer(StackPanel ListePretsView, StackPanel Processeur, TextBlock TempsView, StackPanel ListeBloqueView, StackPanel deroulement, WrapPanel GanttChart)
        {
            ...
            {
                if (listePrets.Count == 0) sort = true; //les premiers processus arrivés => on fait un tri par durée (décroissant)
                ...
                if (sort == true && listePrets.Count != 0) //si un tri par durée est necessaire et il y a des processus prêts
                {
                    listePrets.Sort(
                            delegate (Processus x, Processus y)
                            {
                                ...
                                else return y.duree.CompareTo(x.duree); //sinon, on fait le tri par durée
                            }
        ...
        public override async Task<int> Executer(int tempsDebut, int nbNiveau, Niveau[] niveaux, int indiceNiveau, List<ProcessusNiveau> listeGeneral, List<ProcessusNiveau> listebloqueGenerale, StackPanel[] ListesPretsViews, StackPanel Processeur, TextBlock TempsView, StackPanel ListeBloqueView, StackPanel deroulement)
        {
            ...
            while (listePrets.Count != 0 && PrioNiveaux(niveaux, indiceNiveau, nbNiveau) && !SimulationPage_MultiLvl.stop) //s'il existe des processus prêts et le temps < le temps de fin  ou il n'y a pas de temps fin
            {
                ...
                {
                    listePrets.Sort(delegate (Processus x, Processus y) { return y.duree.CompareTo(x.duree); }); //sinon, on fait le tri par durée
