...
namespace Ordonnancement
{
    public class SlackTime : Ordonnancement
    {
        ...
        public override async Task<int> Executer(StackPanel ListePretsView, StackPanel Processeur, TextBlock TempsView, StackPanel ListeBloqueView, StackPanel deroulement, WrapPanel GanttChart)  // executer la liste des processus 
        {
        ...
        public override int Executer()  // executer la liste des processus 
        {
            ...
            while (listePrets.Count != 0 || indice < listeProcessus.Count || listebloque.Count != 0) //Tant qu'il existe des processus prêts
            {
                ...
                if (listePrets.Count != 0) //S'il y a des processus prêts
                {
                    listePrets.Sort(delegate (Processus x, Processus y)
                    {
                        if (x.slackTime.CompareTo(y.slackTime) == 0) return x.tempsArriv.CompareTo(y.tempsArriv); //si les processus ont le même temps restant, on les trie selon le temps d'arrivée
                        else return x.slackTime.CompareTo(y.slackTime); //sinon, on fait le tri par temps restant
                    }
                                        );
                    listePrets[0].transition = 2; //Activation du 1er processus de listePrets
                    listePrets[0].etat = 2;
        ...
        public override async Task<int> Executer(int tempsDebut, int nbNiveau, Niveau[] niveaux, int indiceNiveau, List<ProcessusNiveau> listeGeneral, List<ProcessusNiveau> listebloqueGenerale, StackPanel[] ListesPretsViews, StackPanel Processeur, TextBlock TempsView, StackPanel ListeBloqueView, StackPanel deroulement,int j)
        {
            bool anime = true, debut = true;
            Processus proc;
            StackPanel ListePretsView = ListesPretsViews[indiceNiveau];
            ...
            //s'il existe des processus prêts et ( On n'est pas encore arrivé à tempsFin ou il n'y a pas de temps fin )
            {
                ...
                await InterruptionExecute(niveaux,listebloqueGenerale, ListesPretsViews, indiceNiveau, ListeBloqueView, Processeur, deroulement, nbNiveau);
                temps++;
                UpdateStackTime(temps);
                TempsView.Text = temps.ToString();
                niveaux[indiceNiveau].indice[0] = await MAJListePrets(temps, niveaux[indiceNiveau].indice[0], niveaux, listeGeneral, indiceNiveau, ListesPretsViews); //remplir la liste des processus prêts de chaque niveau
